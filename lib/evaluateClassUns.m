...
%
% Evaluation of a single class (unsupervised)
%
...
for i = 1:nMasks
    ...
    if ~all(size(mask) == size(gtMask))
        mask = imresize(mask, size(gtMask), 'nearest');
    end
    ...
    PB = PB + sum(gtMask(:)==mask(:)) ./ prod(size(gtMask));
...
for i = 1:nMasks
    ...
    PF = PF + sum(gtMask(:)==mask(:)) ./ prod(size(gtMask));
